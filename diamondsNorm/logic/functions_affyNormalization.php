<?php
/*
 * Author:				Job van Riet + Jordy Coolen + ArrayAnalysis.org (Affy module) Date of creation:		10-4-14 Date of modification:	10-4-14 Version:				1.0 Modifications:			Original version Known bugs:				None known Function:				This file houses the functions to perform Affymetrix normalizations on samples uploaded to the DIAMONDS DB. The actual normalization is done by a slightly altered ArrayAnalysis.org R script for affymetrix normalisation, this script is run as a background deamon.
 */
...
function normalizeAffyStudy($GET, $idStudy, $studyTitle) {
	...
	// dd the idNorm to the output directory (expression/normed/idNorm/)
	...
	if (isset ( $GET ['performStatistics'] ) && $GET ['performStatistics'] == "on") {
		...
		// ake a new normQCResult/idStat folder
	...
	$arguments = ("--createLog TRUE
				--inputDir $inputCELLFolder
				...
				--img.nameInPCA " . (isset ( $GET ['nameInPCA'] ) ? 'TRUE' : 'FALSE') . "
...
// /////////////////////////////////////////////////////////////////
// Main function to perform affy statistics only ///
// /////////////////////////////////////////////////////////////////

// Main function for the normalization of Illumina Beadchip data, this function will call other functions
function doAffyStatistics($GET, $idStudy, $studyTitle) {
	// Make a connection to the DIAMONDSDB (Defined in: functions_dataDB.php)
	$connection = makeConnectionToDIAMONDS ();
	...
	$queryFiles = ("SELECT idFileType, folderName, fileName, idNorm FROM vFilesWithInfo WHERE idStudy = $idStudy AND idNorm = " . $GET ['normSelect'] . " AND idFileType = 36;");
	...
	if (! isset ( $normObjectPath )) {
		$connection->query ( "UPDATE tJobStatus SET status = 2, statusMessage = 'Failed: Could not find the normalized Affy R object in the DB! WHERE idJob = '$idJob'" );
		exit ( "<p><font color=red>Could not find the normalized R object in the DB!</font></p>" );
	} 	// If file not on fileserver
	elseif (! file_exists ( $normObjectPath )) {
		$connection->query ( "UPDATE tJobStatus SET status = 2, statusMessage = 'Failed: Could not find the normalized Affy R object on the fileserver on: $normObjectPath! WHERE idJob = '$idJob'" );
	...
	$connection->query ( "INSERT INTO tFiles (`idStudy`, `idFileType`, `fileName`, idnorm) VALUES ($idStudy, '75', 'descriptionFile.txt', $idStat);" );
