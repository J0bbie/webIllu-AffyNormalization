...
<body onload="showSampleSelection()">
	...
	<div id="form_container">
		...
		<form action="getForm.php" method="GET" name="normalizeStudyForm" id="normalizeStudyForm">
			...
			<input id="formType" name="formType" type="hidden" value="normalizeStudy" /> 
			...
			<div class="form_description">
				<h2>Normalize the samples from this study.</h2>
				<p>This form can be used to normalize the omics data from the samples originating from this study.</p>
			</div>
			<ol>
						...
						if ($result =  mysqli_query($connection, "SELECT count(idStudy) as count FROM tSamples WHERE idStudy = $idStudy AND sxsName != 0 ;")) {
							while ($row = mysqli_fetch_assoc($result)) {
								if($row['count'] != 0){
									...
									echo "<input type='checkbox' id='skipNoSXS' name='skipNoSXS' checked />Skip samples without SXSNumber?<br>";
								}
								else{
