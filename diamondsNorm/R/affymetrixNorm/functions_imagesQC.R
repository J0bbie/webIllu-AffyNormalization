...
coverAndKeyPlot <- function(description=NULL, refName="", WIDTH=1000, 
                            HEIGHT=1414, outputDirectory="") {
  
  if(is.null(description)) stop("The description parameters is required")
  
  #create a 'cover sheet'
  png(file = paste(outputDirectory,"Cover_1.png", sep="/"), width=WIDTH, height=HEIGHT)
  ...
  for(j in 1:noOfPages) {    
    png(file = paste(paste(outputDirectory,"Description", sep="/"),ifelse(refName!="","_",""),refName,
...
QCtablePlot <- function(Data,quality=NULL,sprep=NULL,lys=NULL,
                        ...
                        scaleFact=TRUE, WIDTH=1000, HEIGHT=1414, POINTSIZE=24, outputDirectory="") {
...
samplePrepPlot <- function(Data,sprep=NULL,lys=NULL,plotColors=NULL, 
                           WIDTH=1000, HEIGHT=1414, POINTSIZE=24,MAXARRAY=41, outputDirectory="") {
  ...
  if(!is.null(sprep) && !is.null(lys)) {
    ...
    # create the plot      
    png(file = paste(outputDirectory, "RawDataSamplePrepControl.png", sep="/") ,width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)
...
ratioPlot <- function(Data,quality=NULL,experimentFactor=NULL,plotColors=NULL,legendColors=NULL, 
  ...
  if(!is.null(quality)) {  
    ...
    png(file = paste(outputDirectory,"RawData53ratioPlot_beta-actin.png", sep="/"),width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)
    plotFun(1,3,3,"beta-actin")
    dev.off()
    png(file = paste(outputDirectory,"RawData53ratioPlot_GAPDH.png", sep="/"),width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)
...
RNAdegPlot <- function(Data, Data.rnadeg=NULL, plotColors=NULL, 
  ...
  png(file = paste(outputDirectory,"RawDataRNAdegradation.png", sep="/"), width=WIDTH, height=HEIGHT, pointsize=POINTSIZE)
...
hybridPlot <- function(Data,quality=NULL,plotColors=NULL, 
  ...
  if(!is.null(quality)) {
    ...
    png(file = paste(outputDirectory,"RawDataSpikeinHybridControl.png", sep="/"),width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)
    if(length(sampleNames(Data)) < MAXARRAY){    
      par(mfrow=c(1,1),oma=c(13,0,0,0))
    }else{
      par(mfrow=c(1,1),oma=c(0.1,0,0,0))
...
percPresPlot <- function(Data,quality=NULL,experimentFactor=NULL,plotColors=NULL,legendColors=NULL, 
  ...
  if(!is.null(quality)) {
    ...
    png(file = paste(outputDirectory,"RawDataPercentPresent.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    par(mfrow=c(1,2),oma=c(17,0.5,0,0.5),cex.axis=0.8)
...
PNdistrPlot <- function(Data, WIDTH=1000, HEIGHT=1414, POINTSIZE=24, outputDirectory="") {  
  png(file = paste(outputDirectory,"RawDataPosNegDistribution.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
...
backgroundPlot <- function(Data,quality=NULL,experimentFactor=NULL,plotColors=NULL,legendColors=NULL, 
  ...
  if(!is.null(quality)) {  
    ...
    png(file = paste(outputDirectory,"RawDataBackground.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    par(mfrow=c(1,2),oma=c(17,0.5,0,0.5),cex.axis=0.8)
...
scaleFactPlot <- function(Data,quality=NULL,experimentFactor=NULL,plotColors=NULL,legendColors=NULL, 
  ...
  if(!is.null(quality)) {
    ...
    png(file = paste(outputDirectory,"RawDataScaleFactors.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
...
controlPlots <- function(Data,plotColors=NULL,experimentFactor=NULL,legendColors=NULL,
                         affxplots=TRUE,boxplots=TRUE, WIDTH=1000, HEIGHT=1414, POINTSIZE=24,MAXARRAY=41, outputDirectory="") {
  ...
  if(affxplots) { #plot affx controls
    ...
    if(length(affx1)>0 | length(grep("affx",cntrlVars))>0) {
      ...
      png(file=paste(outputDirectory,"RawDataAFFXControlsProfiles.png", sep="/"), width=250*dimn+250, height=dimnCoef*250*dimn, pointsize=POINTSIZE)
    ...
    if(n > 0) {
      for(i in 1:n) {
        if(!is.null(get(varName))) {
          png(paste(outputDirectory,"/RawData",toupper(cntrlVars[i]),"ControlsBoxplot.png",sep=""), width=WIDTH, height=HEIGHT,pointsize=POINTSIZE)
          par(oma=c(17,0,0,0))
...
boxplotFun <- function(Data, experimentFactor=NULL, plotColors=NULL, legendColors=NULL, normMeth="", 
  ...
  png(file = paste(outputDirectory,paste(Type,"DataBoxplot.png", sep=""), sep="/"),width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)  
...
densityFun <- function(Data, plotColors=NULL, normMeth="", 
  ...
  png(file = paste(outputDirectory,paste(Type,"DensityHistogram.png", sep=""), sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
  if(length(sampleNames(Data))<MAXARRAY){
    cexval <- 0.65
    par(oma=c(12,0,0,0) )
  }else{
...
densityFunUnsmoothed <- function(Data, plotColors=NULL, normMeth="", 
  ...
  png(file = paste(outputDirectory,paste(Type,"DensityHistogramUnsmoothed.png", sep=""), sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
  if(length(sampleNames(Data))<MAXARRAY){
    cexval <- 0.65
    par(oma=c(12,0,0,0) )
  }else{
...
maFun <- function(Data, experimentFactor=NULL, perGroup=FALSE, normMeth="", aType=NULL, 
                  WIDTH=1000, HEIGHT=1414, MAXARRAY=41, outputDirectory=""){                       
  ...
  for(k in (levels(experimentFactor))) {
    ...
    if(length(sampleNames(x2)) < 2) {
      ...
      for(l in 1:nplots){     
        png(paste(outputDirectory,"/",Type,"DataMAplot", ifelse(k!="","_",""), gsub("[:/|\\?<>*\"]","_",k), 
...
plotArrayLayout <- function(Data,aType=NULL, 
                            WIDTH=1000, HEIGHT=1414, POINTSIZE=24, outputDirectory="") {
  ...
  #plot the image
  png(file=paste(outputDirectory,"RawDataReferenceArrayLayout.png", sep="/"),width=WIDTH,height=HEIGHT, pointsize=POINTSIZE)
  par(oma=c(17,2,0,3))
...
spatialImages <- function(Data, Data.pset=NULL, Resid=TRUE, ResSign=TRUE, Raw=TRUE, Weight=TRUE, 
  ...
  splotFun <- function(typeVal){
    ...
    for(l in 1:nPages){
      ...
      png(filename = paste(outputDirectory, "/", "RawData2DVirtualImage_",typeVal,ifelse(nPages>1,paste("_",l,sep=""),""),".png",sep = ""), 
...
array.image<-function(Data, pcut=NULL, relative=TRUE, symm=relative,
  ...
  for(l in 1:nPages){
    from <- nPerPage*l - (nPerPage-1)
    to <-   min(nPerPage*l, length(sampleNames(Data)))   	
    png(filename = paste(outputDirectory, "/", "RawDataArray.image",postfix,
...
PNposPlot <- function(Data, 
                      WIDTH=1000, HEIGHT=1414, POINTSIZE=24, outputDirectory="") {
  png(file = paste(outputDirectory, "RawDataPosNegPositions.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
  par(oma=c(17,0,0,0),srt=90) 
  borderQC2(Data)
...
nuseFun <- function(Data, Data.pset=NULL, experimentFactor=NULL, plotColors=NULL, legendColors=NULL, 
  ...
  png(file = paste(outputDirectory, "RawDataNUSE.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)   
  par(oma=c(17,0,0,0), cex.axis=1)   
...
rleFun <- function(Data, Data.pset=NULL, experimentFactor=NULL, plotColors=NULL, legendColors=NULL, 
  ...
  png(file = paste(outputDirectory, "RawDataRLE.png", sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)   
  par(oma=c(17,0,0,0), cex.axis=1)  
...
correlFun <- function(Data, clusterOption1="pearson", clusterOption2="ward", normMeth="",
                      experimentFactor=NULL, legendColors=NULL, WIDTH=1000, HEIGHT=1414, POINTSIZE=24,MAXARRAY=41, outputDirectory=""){  
  ...
  if(length(sampleNames(Data))<2) {
    ...
    png(file = paste(outputDirectory, paste(Type,"DataArrayCorrelation.png",sep=""), sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    if(length(sampleNames(Data))<MAXARRAY) {
...
clusterFun <- function(Data, experimentFactor=NULL, clusterOption1="pearson", clusterOption2="ward", normMeth="", 
                       plotColors=NULL, legendColors=NULL, plotSymbols=NULL, legendSymbols=NULL, WIDTH=1000, HEIGHT=1414, POINTSIZE=24, MAXARRAY=41, outputDirectory="") {
  ...
  if(length(sampleNames(Data))<3) {
    ...
    png(file = paste(outputDirectory, paste(Type,"DataCluster_",clusterOption1,".png",sep=""), sep="/"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    if(length(sampleNames(Data))<MAXARRAY) {
      cexval1 <- 0.75
      cexval2 <- 1.23
      cexval3 <- 0.55
...
pcaFun <- function(Data, experimentFactor=NULL, normMeth="", scaled_pca=TRUE, plotColors=NULL, legendColors=NULL,
                   plotSymbols=NULL, legendSymbols=NULL, namesInPlot=FALSE, groupsInLegend=TRUE, WIDTH=1000, HEIGHT=1414, POINTSIZE=24, outputDirectory=""){
  ...
  if(length(sampleNames(Data))<3) {
    ...
    if(!is.null(pca1)) {
      ...
      png(file = paste(outputDirectory, paste(Type,"DataPCAanalysis.png",sep=""), sep="/"), width=WIDTH+200*(!namesInPlot), height=HEIGHT+283*(!namesInPlot),
          pointsize=POINTSIZE)
