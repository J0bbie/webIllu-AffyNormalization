...
#Debug parameters
#userParameters <- getArguments(c("--statisticsDir", "/var/www/normdb/data/2_affyStudy/2_CELTEST/21", "--idStudy", 2, "--samplePrep", TRUE, "--inputDir", "/var/www/normdb//data/2_CELTEST/expressionData/raw/", "--outputDir", "/var/www/normdb/data/2_CELTEST/expressionData/normed/8"))
...
if(userParameters$loadOldNorm == FALSE){
  ##################################################################################
  ...
  if(length(grep(".CEL",toupper(colnames(description)[1]), ignore.case = TRUE))>0) {
    message <- (paste("The description file may not contain a header, as the first", "column header seems to be a CEL file name"))
    if(userParameters$createLog) sink()
    stop(message)
  }
  ...
  if(userParameters$rawDataQC){
    ...
    #################################################################################
    #                 Open subset file if user selected a subset for statistics     #
    #################################################################################
    ...
    #################################################################################
    #                         Make subset of samples used for statistics            #
    #################################################################################
    
    if(userParameters$rawDataQC && userParameters$statSubset){
      cat("\nMaking subset of samples in raw data for statistics only.\n")
      
      #Get the groups from the description file and add to the statFile dataFrame
      statFile$groups <- description$FactorValue[match(statFile[,1], description$SourceName)]
      
      #Make generic name for statnames
      statFile$arraySampleNames <- make.names(statFile[,1])
      
      cat("\nSuccesfully read in the statFile containing the samples for the subset!\n")
      
      #Match sampleNames from datafile with first column from description file
      matchedSamplesOnArray <- match(statFile$arraySampleNames,sampleNames(normData))
      
      #Remove the samples not on the rawData object
      rawDataStat <- rawData[,na.omit(matchedSamplesOnArray)]
    ...
    # Make a duplicate of the rawData where statistics will be performed on, if not already made by statSubset parameter
    if(userParameters$statSubset == FALSE){
      rawDataStat <- rawData
    }
    ...
    if(userParameters$samplePrep || userParameters$ratioPlot || userParameters$hybridPlot || userParameters$percPres || userParameters$bgPlot || userParameters$scaleFact) {
      ...
      quality <- NULL
      try(quality <- qc(rawDataStat),TRUE) # calculate Affymetrix quality data for PMMM
      if(is.null(quality)) {
      ...
      if(userParameters$samplePrep) {    
        # find the data 
        try(yack <- yaqc(rawDataStat),TRUE)
        if(exists("yack")) {
        ...
        try({calls<-detection.p.val(rawDataStat)$call
        ...
        if(!exists("lys")) {
          ...
          if(length(lys) > length(sampleNames(rawDataStat))) { lys<-lys[1,] }
        }
      }
      
      QCtablePlot(rawDataStat,quality,sprep,lys,samplePrep=userParameters$samplePrep,ratio=userParameters$ratioPlot,
    ...
    if(userParameters$samplePrep && !is.null(sprep) && !is.null(lys)) {
      ...
      samplePrepPlot(rawDataStat,sprep,lys,plotColors,
    ...
    if(userParameters$ratioPlot && !is.null(quality)) {
      ...
      ratioPlot(rawDataStat,quality=quality,experimentFactor,plotColors,legendColors,
                WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$degPlot) {
      ...
      RNAdegPlot(rawDataStat,plotColors=plotColors,
                 WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$hybridPlot && !is.null(quality)) {
      ...
      hybridPlot(rawDataStat,quality=quality,plotColors,
                 WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$bgPlot && !is.null(quality)) {
      ...
      backgroundPlot(rawDataStat,quality=quality,experimentFactor,plotColors,legendColors,
                     WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$percPres && !is.null(quality)) {
      ...
      percPresPlot(rawDataStat,quality=quality,experimentFactor,plotColors,legendColors,
                   WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$PMAcalls) {
      if(userParameters$useCustomAnnotation) {
        if(userParameters$species=="") {
          ...
          userParameters$species <- deduceSpecies(rawDataStat@annotation)
        }
        if(userParameters$species!=""){
          PMAtable <- computePMAtable(rawDataStat, userParameters$useCustomAnnotation,userParameters$species,userParameters$CDFtype)
        }else{
          warning("\nCould not define species; the CDF will not be changed\n")
          PMAtable <- computePMAtable(rawDataStat,userParameters$useCustomAnnotation)
        }
      } else {
        PMAtable <- computePMAtable(rawDataStat,userParameters$useCustomAnnotation)
      }
      ...
      if(!is.null(PMAtable)) {
        print ("\nWriting PMA table\n")
        write.table(PMAtable, paste(userParameters$statisticsDir, "PMAtable.txt", sep="/"), sep="\t", row.names=FALSE, 
    ...
    if(userParameters$controlPlot) {
      ...
      controlPlots(rawDataStat,plotColors,experimentFactor,legendColors,
                   WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$scaleFact && !is.null(quality)) {
      print ("\nPlot scale factors\n")
      scaleFactPlot(rawDataStat,quality=quality,experimentFactor,plotColors,
                    legendColors,
                    WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$boxplotRaw){
      ...
      boxplotFun(Data=rawDataStat, experimentFactor, plotColors, legendColors,
                 WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$densityRaw){
      ...
      densityFun(Data=rawDataStat, plotColors,
                 WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$MARaw){
      ...
      maFun(Data=rawDataStat, experimentFactor, perGroup=(userParameters$MAOption1=="group"), 
            aType=aType,
    ...
    if(userParameters$layoutPlot) {
      ...
      plotArrayLayout(rawDataStat,aType,
                      WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$spatialImage || userParameters$PLMimage || userParameters$Nuse || userParameters$Rle) {
      ...
      rawDataStat.pset <- fitPLM(rawDataStat)                     
    }
    ...
    if(userParameters$spatialImage) {  
      cat ("\n2D virtual images\n")
      valtry<-try(userParameters$spatialImages(rawDataStat, Data.pset=rawDataStat.pset, TRUE,FALSE,FALSE,FALSE, 
                                               WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir),
                  silent=TRUE)
      if(class(valtry)=="try-error") {
        cat("\nUse array.image instead of spatialImages function\n")
        if(length(sampleNames(rawDataStat))>6){
          # Usage of a median array is interesting when there are enough arrays
          array.image(rawDataStat,WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir)
        }else{
          # Usage when few arrays in dataset (one page for 3 arrays -> max: 2 pages)
          array.image(rawDataStat,relative=FALSE,col.mod=4,symm=TRUE,
                      WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir)
        }
      }
    ...
    if(userParameters$PLMimage) {  
      ...
      valtry<-try(spatialImages(rawDataStat, Data.pset=rawDataStat.pset, TRUE, TRUE, TRUE, TRUE,
                                WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir),
                  silent=TRUE)
      if(class(valtry)=="try-error") {
    ...
    if(userParameters$Nuse){
      print ("\nNUSE boxplot\n")
      nuseFun(rawDataStat, Data.pset=rawDataStat.pset, experimentFactor, plotColors, 
              legendColors,
              WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$Rle){          
      print ("\nRLE boxplot\n")
      rleFun(rawDataStat, Data.pset=rawDataStat.pset, experimentFactor, plotColors, 
             legendColors,
             WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$correlRaw){
      ...
      correlFun(Data=rawDataStat, experimentFactor=experimentFactor, legendColors=legendColors,
                WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$PCARaw){
      ...
      pcaFun(Data=rawDataStat, experimentFactor=experimentFactor, 
             plotColors=plotColors, legendColors=legendColors, plotSymbols=plotSymbols,
             legendSymbols=legendSymbols, namesInPlot=userParameters$img.nameInPCA,
             WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir)
    ...
    if(userParameters$clusterRaw){
      ...
      clusterFun(Data=rawDataStat, experimentFactor=experimentFactor,
...
if(userParameters$normDataQC){
  ...
  if(userParameters$loadOldNorm){
    ...
    cat("\nLoading old normalized data succesfull\n")
    
    cat("\nChanging description file based on samples on the normalized R data object\n")
    
    matchedSamplesDescription <- match(sampleNames(normData), description$arraySampleNames)
    
    description = description[na.omit(matchedSamplesDescription),]
    
    cat("\nDescription file now only contains the samples on the normalized R LumiBatch object\n")
  ...
  if(userParameters$loadOldNorm && userParameters$perGroup){
    ...
    #Match sampleNames from datafile with first column from description file
    file_order2 <- match(description2$arraySampleNames,sampleNames(normData))
  ...
  if(userParameters$normDataQC && userParameters$statSubset){
    cat("\nMaking subset of samples in norm data for statistics only.\n")
    ...
    #Remove the samples not on the normData object
    normData <- normData[,na.omit(matchedSamplesOnArray)]
    ...
    cat("\nSuccesfully made subset of samples in norm data!\n")   
  }
  
  #################################################################################
  # Create array groups, array names and plot variables for old normed data       #
  #################################################################################
  
  cat("\nCreating a plot colorset for each array group based on description file.\n")
  ...
  if(userParameters$densityNorm){
    ...
    densityFun(Data=normData, plotColors, normMeth=userParameters$normMeth,
               WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    addStatFile( userParameters$idStudy, 71, userParameters$idStatistics, "NormDensityHistogram.png")
  }
  ...
  if(userParameters$MANorm){
    ...
    addStatFile( userParameters$idStudy, 69, userParameters$idStatistics, "NormDataMAplot.png")
  }             
  ...
  if(userParameters$correlNorm){
    ...
    correlFun(Data=normData, normMeth=userParameters$normMeth, experimentFactor=experimentFactor, legendColors=legendColors,
              WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,MAXARRAY=userParameters$img.maxArray,outputDirectory=userParameters$statisticsDir)
    addStatFile( userParameters$idStudy, 66, userParameters$idStatistics, "NormDataArrayCorrelation.png")
  }
  ...
  if(userParameters$PCANorm){
    ...
    pcaFun(Data=normData, experimentFactor=experimentFactor,normMeth=userParameters$normMeth, 
           ...
           legendSymbols=legendSymbols, namesInPlot=userParameters$img.nameInPCA,
           WIDTH=userParameters$img.width,HEIGHT=userParameters$img.height ,POINTSIZE=userParameters$img.pointSize,outputDirectory=userParameters$statisticsDir)
    addStatFile( userParameters$idStudy, 70, userParameters$idStatistics, "NormDataPCAanalysis.png")
