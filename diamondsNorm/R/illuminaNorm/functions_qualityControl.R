...
#Date of  creation:           28-2-14
#Date of modification:        28-2-14
#Version:                     1.0
#Modifications:               Original version
#Known bugs:                  None known
#Function:                    Contains functions for filtering and background correction of Illumina expression data.

###################################################
#                   Functions                     #
###################################################

#Loads the raw illumina beadarray data
#Performs QC if rawDataQC == True
#Returns a lumiBatch object with the raw data
import.rawData <-function (expData, detectionTh ,convertNuID, checkDupId, lib.mapping, dec, parseColumnName, rawDataQC) {
...
#Returns a lumiBatch object with the normalized data.
lumi.normData <-function(x.lumi, bg.correct, bgCorrect.m,variance.stabilize, variance.m,normalize, normalization.m, normDataQC) {
          normData = lumiExpresso(
                    x.lumi, 
...
#Create summary file from lumi.batch QC object containing: mean, standard deviation, detection rate(0.01) and distance to sample mean
#Need a lumiBatch object and a file destination
...
createAnnoFun <- function(df, lib.mapping, lib.All.mapping) {
  #list of nuIDs 
  nuIDs = rownames(df)
