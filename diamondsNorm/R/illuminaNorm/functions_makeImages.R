...
#Known bugs:                  cv.plot does not work if a low amount of samples are supplied, therefore a tryCatch() is set there.
#Function:                    This script contains the functions to create the plots used in the normalization and QC of Illumina Beadchip data.
...
clusterFun <- function(x.lumi, normalized=FALSE ,experimentFactor=NULL, 
                       ...
                       WIDTH=1000, HEIGHT=1414, POINTSIZE=24,MAXARRAY=41, normalization.m=NULL, fileName) {
  ...
  if(length(sampleNames(x.lumi))<3) {
    ...
    png(file = paste(fileName,"_",Type,"_","dataCluster_",clusterOption1,"_",clusterOption2,".png",sep=""),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    ...
    if(length(levels(experimentFactor))>1) { 
      legend("topright",levels(experimentFactor),
             pch=legendSymbols,col=legendColors, ncol=3)
    }
    par(cex=cexval3)    
    dev.off()
  }
...
pcaFun <- function(x.lumi, normalized=FALSE, experimentFactor=NULL, 
                   ...
                   WIDTH=1000, HEIGHT=1414, POINTSIZE=24, normalization.m=NULL, fileName){  
  ...
  # Check if the groups are given (to base the colorscheme on)
  ...
  # Check if more than 3 samples are given
  if(length(sampleNames(x.lumi))<3) {
    ...
    # Check if PCA has been succesfull and if so, plot on .png
    if(!is.null(pca1)) {
      ...
      # Create .png file
      png(file = paste(fileName ,Type,"dataPCA_analysis.png",sep="_"), width=WIDTH+1500, height=HEIGHT+500,
          pointsize=POINTSIZE)
      ...
      par(oma=c(10,10,10,10) , xpd=TRUE)
      
      # Plot the first three principal components of the PCA
      ...
      if(namesInPlot){ 
        text(pca1$x[,1],pca1$x[,2], sampleNames(x.lumi),pos=4,cex=cex.text,col=tcol)
      }
      ...
      if(namesInPlot){ 
        text(pca1$x[,1],pca1$x[,3], sampleNames(x.lumi),pos=4,cex=cex.text,col=tcol)
      }
      ...
      if(namesInPlot){
        text(pca1$x[,2],pca1$x[,3], sampleNames(x.lumi),pos=4,cex=cex.text,col=tcol)
      }
      
      # Plot a barplot showing the total explained variance per principal component
      barplot((100*pca1$sdev^2)/sum(pca1$sdev^2),xlab="components",ylab="% of total variance explained")
      
      # Determine whether the groups should be added atop the legend
      groupsAdded <- FALSE
      if(groupsInLegend) {
        if(length(levels(experimentFactor))>1) {
          groupsAdded <- TRUE
      ...
      if(namesInPlot) {
        ...
        if(groupsAdded) {
          legend("topright", c(levels(experimentFactor),"",sampleNames(x.lumi)), ncol=3,
          ...
          legend("topright", sampleNames(x.lumi),pch=plotSymbols,
...
correlFun <- function(x.lumi, normalized=FALSE, 
  ...
  if(length(sampleNames(x.lumi))<2) {
    ...
    #print(Type)
    #print(normalized)
    png(file = paste(fileName, Type,"dataArrayCorrelation.png",sep="_"),width=WIDTH+500,height=HEIGHT,pointsize=POINTSIZE)
    if(length(sampleNames(x.lumi))<MAXARRAY) {
      par(oma=c(17,0,0,5),cex.axis=0.7,cex.main=0.8)
      #subval <- 10
    } else {
      par(oma=c(17,0,0,5),srt=90,las=2,cex.axis=0.5,cex.main=0.8)
      #subval <- 16
    }        
