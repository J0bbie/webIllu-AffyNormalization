...
pcaFun <- function(x.lumi, normalized=FALSE, experimentFactor=NULL, 
  ...
  if(length(sampleNames(x.lumi))<3) {
    ...
    if(!is.null(pca1)) {
      ...
      png(file = paste(fileName ,Type,"dataPCA_analysis.png",sep="_"), width=WIDTH+200*(!namesInPlot), height=HEIGHT+283*(!namesInPlot),
          pointsize=POINTSIZE)
...
correlFun <- function(x.lumi, normalized=FALSE, 
  ...
  if(length(sampleNames(x.lumi))<2) {
    ...
    #print(Type)
    #print(normalized)
    png(file = paste(fileName, Type,"dataArrayCorrelation.png",sep="_"),width=WIDTH,height=HEIGHT,pointsize=POINTSIZE)
    if(length(sampleNames(x.lumi))<MAXARRAY) {
