...
if(userParameters$performStatistics){
     ...
     if(userParameters$normDataQC){
          ...
          if(userParameters$loadOldNorm){
               ...
               matchedSamplesDescription <- match(description$arraySampleNames,sampleNames(normData))
               
               description <- description[,na.omit(matchedSamplesDescription)]
               
               cat("\nChanging description complete\n")
          ...
          if(userParameters$loadOldNorm && userParameters$perGroup){
               ...
               normData <- normData[,na.omit(matchedSamplesDescription)]
          ...
          if(userParameters$normDataQC && userParameters$statSubset){
               ...
               matchedSamplesStat <- match(statFile[,1], x)
               
               #Make subset of samples
               normData <- normData[, matchedSamplesStat]
